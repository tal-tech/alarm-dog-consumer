...
class ConditionArr
{
    ...
    {
        // 适配旧数据
        $keys = $ruleItem['field_split'] ??
            (is_array($ruleItem['field']) ?
                $ruleItem['field'] : self::fieldSplit($ruleItem['field']));

        [$exist, $value] = static::getValue($entry, $keys);
        ...
        switch ($ruleItem['operator']) {
            ...
            case static::OP_CONTAIN:
                return ! is_null($value) && mb_strpos($value, $ruleItem['threshold']) !== false;
